...
pub struct WordListNode<Term>
where
    Term: std::cmp::Eq + std::hash::Hash,
{
...
impl<T: std::cmp::Eq + std::hash::Hash + std::fmt::Debug> WordListNode<T> {
    ...
    pub fn find_terms(&self, query: &str, kind: SearchKind) -> Vec<Rc<T>> {
                                ...
                                match pri {
                                    FuzzPriority::WordBoundary => {
                                        ...
                                        match self.hope_for_success(&c, 1) {
                                            Some(alt_word) => {
                                                let new_candidates = HashSet::from_iter(
                                                    alt_word.collect_terms(None).into_iter(),
                                                );
                                                if new_candidates.intersection(&restrict_to).count()
                                                    > 0
                                                {
        ...
        let rslt: Vec<Rc<T>> = res
            .into_iter()
            .unique()
            .filter(|x| {
                if restrict_to.len() > 0 {
                    restrict_to.contains(x)
                } else {
                    true
                }
            })
        ...
        if rslt.len() == 0 {
            match kind {
                SearchKind::Fuzzy(fuzz, pri) => match pri {
                    FuzzPriority::TypoCorrection => {
                        return self
                            .find_terms(query, SearchKind::Fuzzy(fuzz, FuzzPriority::WordBoundary))
                    }
                    _ => (),
                },
                _ => (),
...
#[cfg(test)]
mod tests {
    use super::{FuzzPriority, SearchKind, WordDict};
